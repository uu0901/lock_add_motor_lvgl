...
void debug_upload_data(debug_data *data, uint8_t upload_type)
{
    ...
    switch (upload_type)                                                /* 判断数据类别 */
    {
        ...
        case TYPE_SPEED:                                                /* 电机速度 */
            ...
            upload_data[cur_data++] = (data->speed >> 8) & 0xFF;        /* 先存入速度值高8位（小端模式中u16赋给u8类型，只取低8位） *///小端模式：https://blog.csdn.net/weixin_45633061/article/details/117421452
...
//        HAL_UART_Transmit(&g_uart1_handle, upload_data, cur_data, 0xFFFF);     /* 发送数据到上位机 */
        ...
        uart1_SendArray(upload_data,cur_data);                             /* 发送数据到上位机 */
    }
}
