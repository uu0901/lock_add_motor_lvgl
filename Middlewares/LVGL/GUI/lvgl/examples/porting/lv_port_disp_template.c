...
static lv_color_t buf_1[MY_DISP_HOR_RES * 480]  __attribute__((at(0x68000000 + 10*1024 + 320 * 2)));//此处偏移是malloc.h里的mem2内存参数设定，10*1024+（10*1024/32）*（malloc.c文件里的内存管理表的数据类型是uint16占用了2个字节，所以是2）
static lv_color_t buf_2[MY_DISP_HOR_RES * 480]  __attribute__((at(0x68000000 + 800 * 480 *2 + 10*1024 + 320 * 2)));
/**********************
 *      TYPEDEFS
 **********************/
...
//    static lv_color_t buf_1[MY_DISP_HOR_RES * 10];                                              /* 设置缓冲区的大小为 10 行屏幕的大小 */
    lv_disp_draw_buf_init(&draw_buf_dsc_1, buf_1, NULL, MY_DISP_HOR_RES * 480);                  /* 初始化显示缓冲区 */
#endif
