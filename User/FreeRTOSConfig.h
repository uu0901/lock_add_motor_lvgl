...
//#define configSYSTICK_CLOCK_HZ                          (configCPU_CLOCK_HZ / 8)/* 定义SysTick时钟频率，当SysTick时钟频率与内核时钟频率不同时才可以定义, 单位: Hz, 默认: 不定义 */，这里属于是自定义分频的概念了，systic要和cpu频率同频，如果systic分配了。就要在这里开启对应的cpu分频,之后将SYstic的频率=设定的cp，若systic没分频就关闭这个宏定义，config会自动帮我们配置systic=cpu
